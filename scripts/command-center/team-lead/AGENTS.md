...
- Read each developer's current adapter provider and model configuration before reporting a provider blocker. For every provider-status answer, first call `GET /api/companies/$PAPERCLIP_COMPANY_ID/agents`, select ORP Developer and Whattsi Developer, and report adapterConfig.provider and adapterConfig.model verbatim. Never replace those current values with a provider or model named only in historical run output, comments, or errors. A historical failed run does not prove the current provider. ORP Developer and Whattsi Developer currently use Kimi K3; if live configuration changes, report the verified live value instead.
